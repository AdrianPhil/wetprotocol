...
public class Drop extends ApplicationAdapter {
	...
	public void render() {
		...
		if (Gdx.input.isTouched()) {
			...
			if (touchPos.x > bucket.x && touchPos.x < bucket.x + 64 && touchPos.y > bucket.y
					&& touchPos.y < bucket.y + 64) {
				bucket.x = touchPos.x - 64 / 2;
				bucket.y = touchPos.y - 64 / 2;
			} else if (touchPos.x > raindrop.x && touchPos.x < raindrop.x + 64 && touchPos.y > raindrop.y
					&& touchPos.y < raindrop.y + 64) {
				raindrop.x = touchPos.x - 64 / 2 ;
				raindrop.y = touchPos.y - 64 / 2;
			}
		}
